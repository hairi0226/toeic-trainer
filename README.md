...
- 题库：Day 1–6（对应打印版 PDF，144 题）+ 按 ETS 真题结构出的专项套题（Part 5 ×2、Part 6、Part 7 新题型，95 题），全部是原创 TOEIC-style 仿真题，**不是 ETS 官方真题**；每题带解析、翻译、生词
...
- app 里的题全部是原创仿真题；**官方真题请在纸上做**，来源见首页「官方免费资料」（ETS 官方样题 PDF；韩国 TOEIC 委员会每年上下半年公开一次真题，免费，配 MP3）。
- 做完后到「录纸面 → 官方真题」：选题号模板（整卷 101–200 / 只做 Part 5 / ETS 样题），点错题号、填用时，正确率按 Part 统计，首页和错题本都能看到。
- app 不存官方题目内容（版权归 ETS），只记题号和成绩。
...
出题按 `docs/出题规范.md`（照 ETS 真题结构）。规则（详见 `CLAUDE.md`）：题目 id 永不改、永不复用；改了答案或选项要把该题 `rev` +1；下架题目用 `"retired": true` 而不是删除。
